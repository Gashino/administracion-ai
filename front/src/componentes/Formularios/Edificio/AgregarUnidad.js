...
import { agregarUnidadService } from '../../../services/servicioUnidad';



const AgregarUnidad = ({changeValue,edificio}) => {
    ...
    const handleAceptar = () => {
        agregarUnidadService(edificio.codigo,piso,unidad,setError)
    };
    ...
    return (
        <div style={formStyleEdificio}>
            ...
            {error === "error" && <Alert severity="error" variant='filled' sx={{mt:'240px'}}>Error al agregar unidad!</Alert>}
            {error === "pass" && <Alert severity="success" variant='filled'sx={{mt:'240px'}}>Unidad agregada!</Alert>}
        </div>
    );
};
